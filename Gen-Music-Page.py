import os,re,glob,pyperclip,datetime,time
def Getinf(FilePath):
...
<!-- more -->
此页面由[Python代码](https://github.com/panxiuwen/Gen-Music-Page)自动生成，更新于'''
    t2=t2+datetime.datetime.strftime(datetime.datetime.now(),'%Y 年%m 月%d 日，')
    a=0
    for i in list:
            Dir=i[0]
            ...
            for file in files:
                t=t+Getinf(file)+"\n"
                a=a+1
    t2=t2+"乐库共"+str(a)+"首歌，在线版为[潘修文的乐库](http://panxiuwen.dynv6.net/)，建在基于校园网IPV6的博主PC上，所以不一定能访问\n"+t
    print(a)
    pyperclip.copy(t2)
scan()
